...
class JobForm(forms.Form):
    ...
    link_to_company = forms.URLField(label='Link to Company', max_length=200, widget=forms.URLInput(attrs={'class': 'form-control', 'placeholder': 'Enter Link to Company Page'}))
    job_type = forms.ChoiceField(choices=JobType.choices, label='Job Type', widget=forms.Select(attrs={'class': 'form-control'}))
    skill_1 = forms.ModelChoiceField(queryset=JobSkills.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
    skill_2 = forms.ModelChoiceField(queryset=JobSkills.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
    skill_3 = forms.ModelChoiceField(queryset=JobSkills.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
    skill_4 = forms.ModelChoiceField(queryset=JobSkills.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
    skill_5 = forms.ModelChoiceField(queryset=JobSkills.objects.all(), widget=forms.Select(attrs={'class': 'form-control'}))
